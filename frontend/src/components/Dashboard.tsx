...
export default function Dashboard() {
  ...
  const [selectedMetric, setSelectedMetric] = useState<
    "hs_rate" | "accuracy" | "kills" | "deaths" | "duration_minutes" | "kd"
  >("hs_rate");
  ...
  const uniqueExercises = Array.from(
    new Set(sessions.map((s) => s.exercise_type).filter(Boolean))
  ) as string[];
  const roleRaw =
  ...
  async function approveRenewal(invitationId: string) {
    try {
      const res = await fetch(
        `/api/invitations/${invitationId}/approve-renewal`,
        { method: "POST" }
      );
      const json = await res.json();
            ...
            <div className="mb-8 bg-zinc-950/80 backdrop-blur-xl border-2 border-zinc-800/50 rounded-lg p-4 shadow-xl">
              ...
              <div className="space-y-3">
                {notifications.map((n) => (
                  <div
                    key={n.id}
                    className="flex items-center justify-between bg-zinc-900/60 border border-zinc-800 rounded-md p-3"
                  >
                    ...
                    <div className="flex gap-2">
                      {n.related_invitation_id && (
                        <button
                          onClick={() =>
                            approveRenewal(n.related_invitation_id)
                          }
                      ...
                      <button className="px-3 py-2 bg-zinc-800 hover:bg-zinc-700 text-white rounded-md text-sm">
                        Annuler
                      </button>
                    </div>
                  </div>
          ...
          {selectedPlayer ? (
                ...
                <div className="p-6 border-b border-zinc-800 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {uniqueExercises.length > 0 && (
                      <>
                        <label className="text-sm text-zinc-400 ml-3">
                          Exercice
                        </label>
                        <select
                    ...
                    <tbody className="divide-y divide-zinc-800">
                      {sessions.length === 0 ? (
                        <tr>
                          ...
                          >
                            No training sessions yet. Click "New Session" to add
                            one.
                          </td>
                        </tr>
                      ) : (
            ...
            <div className="bg-zinc-950/80 backdrop-blur-xl border-2 border-zinc-800/50 rounded-lg shadow-xl shadow-black/30 p-12 text-center">
              ...
              <p className="text-zinc-400 mb-6">
                Add your first player to start tracking performance
              </p>
              <button
                onClick={() => setShowAddPlayer(true)}
          ...
          <div className="bg-zinc-950/90 backdrop-blur-xl border-2 border-zinc-800/50 rounded-lg shadow-2xl shadow-black/50 w-full max-w-md">
            <div className="flex items-center justify-between p-6 border-b border-zinc-800">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-bold text-white">
                  Message à {selectedPlayerData.player_name}
                </h2>
              </div>
              <button
